...
let
  ...
  # wespeaker speaker-embedding ONNX checkpoint sherpa-onnx's diarizer uses
  # to tell speakers apart. Upstream's own release tag
  # ("speaker-recongition-models") is misspelled; kept verbatim below since
  # it's their typo, not ours.
  sherpa-embedding-src = pkgs.fetchurl {
...

{
  ...
  # Single directory holding both sherpa-onnx diarization checkpoints,
  ...
  speaker-models = pkgs.runCommand "speaker-models" { } ''
    ...
    cp ${sherpa-embedding-src} $out/embedding.onnx
  '';
}
