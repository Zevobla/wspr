...
//! -- `general` (backend pickers, launch-at-login, sound) and `devices`
//! (microphone/hotkey preview) today, with further groups added alongside
//! this doc comment as more of `Config` gets exposed here instead of
//! requiring a hand-edited `config.toml`.
//!
...
pub(super) fn view<'a>(state: &'a State, scheme: &'static color::Scheme) -> Element<'a, Message> {
    column![general::view(state, scheme), devices::view(state, scheme),]
        .spacing(spacing::XL)
        .into()
}
