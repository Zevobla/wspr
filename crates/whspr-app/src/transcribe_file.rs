...
pub async fn run_transcribe(file: PathBuf, config: Config) -> Result<(String, f32), String> {
...
/// Returns the recognized text alongside the recorded audio's duration, so
/// callers can save a complete history entry (see `crate::history`) instead
/// of just the text.
