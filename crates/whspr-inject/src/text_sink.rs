//! Text delivery: [`EnigoTextSink`], the `whspr_core::TextSink` that pastes
//! dictated text into the focused app via the clipboard, falling back to
//! synthetic typing. Split out of `lib.rs` so the hotkey listener and the
//! injection path each stay a readable size.
...
use whspr_core::{Result, TextSink, WhsprError};

use crate::clipboard::{stage_and_paste, ArboardClipboard, PasteOutcome};
